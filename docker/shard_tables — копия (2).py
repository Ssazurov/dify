...
def is_toc_table(headers, rows, threshold=0.7):
    ...
    Если первая колонка повторяется более чем в threshold доле строк, считаем оглавлением.
    """
    if not rows or len(rows) < 2:
        return False
    first_col_values = [row[0] if row else "" for row in rows]
    # считаем наиболее частый элемент
    from collections import Counter
    counter = Counter(first_col_values)
    most_common = counter.most_common(1)
    if not most_common:
        return False
    top_value, count = most_common[0]
...
def convert(a):
    ...
    # Стек заголовков для генерации осмысленного slug
    heading_stack = []          # храним заголовки по порядку
    pending_heading = ""        # последний заголовок (для обратной совместимости)
    last_added_heading = ""     # для предотвращения дублирования
    ...
    for idx, (item, _level) in enumerate(doc.iterate_items()):
        ...
        if isinstance(item, TableItem):
            ...
            # Генерируем slug из стека заголовков (берём последние 3 уровня)
            if heading_stack:
                # очищаем от пустых и делаем slug
                clean_headings = [slugify(h, max_words=3) for h in heading_stack[-3:] if h.strip()]
                slug_base = "_".join(clean_headings) if clean_headings else "table"
            else:
                slug_base = "table"

            # используем текущий pending_heading как заголовок для подписи
            ...
            # Проверяем, не является ли таблица оглавлением
            if is_toc_table(headers, rows, threshold=0.7):
                # Вместо шардирования добавляем как обычный markdown-список (или сохраняем как есть)
                print(f"🔖 Таблица {table_counter} ('{heading}') определена как оглавление — сохраняем как список")
                # Можно добавить заголовок и сам список
                out_lines.append(f"\n### {heading} (оглавление)\n")
                # Превратим строки в список пунктов
                for r in rows:
                    # соединяем колонки через " – "
                    row_text = " – ".join(r)
                    out_lines.append(f"- {row_text}")
                continue

            # Обычная таблица — шардируем
            out_lines.append(f"\n### {heading}\n")
            ...
            if label in ("section_header", "title"):
                # Обновляем стек заголовков (упрощённо: добавляем, если не дублируется)
                if text != last_added_heading:
                    heading_stack.append(text)
                    if len(heading_stack) > 10:   # ограничим глубину
                        heading_stack.pop(0)
    ...
    # Если выходной путь — директория, создаём файл с именем исходного
